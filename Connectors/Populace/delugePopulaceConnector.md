...
Revision Timestamp: 2026-03-19T18:31:35.916Z
Status: Functional
---
...
- **Output:** A Map containing `{"success": true/false, "data": "response text", "error_message": "details"}`.
- **Primary Entities:** 
...
The script maintains an internal registry of supported actions. Each entry defines the HTTP verb and the URL path template. Path templates use curly braces (e.g., `{userId}`) which are dynamically replaced by values in the input payload.
...
> [!IMPORTANT]
> This script modifies the `payload` map in-place during the URL construction phase. If a key is used as a path parameter, it will not be present in the final JSON body sent to the API.
...
> [!CAUTION]
> The script currently returns a Map literal while the signature is defined as `string`. While Deluge handles this loosely, future strict-type updates might require wrapping the return in a `.toString()`.
